...
const AddNote = () => {
  ...
  const saveInDb = async (title , content , type) =>{
    try {
      switch (type) {
        case 'normal':
          await database.runAsync("INSERT INTO notes (title , type , content) VALUES (?,?,?);",[title , type , content])
        break;
        default:
          try {
            const queryResult = await database.runAsync("INSERT INTO notes ( title , type ) VALUES (?,?);",[title , type])
            console.log('enregistrement des données reussi dans la table notes')
            content.map(async (item)=>{
              try {
                const note_id = queryResult.lastInsertRowId
                await database.runAsync("INSERT INTO items ( note_id , name ,is_checked ) VALUES (?,?,?);",[note_id , item.text , item.checked])
                console.log('enregistrement des données reussi dans la table items')
              } catch (error) {
                console.error(error)   
              }
            })
